...
const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => {
  ...
  const logs = [
    ...
    "Mounting clinical PRISMA filesystem...",
    "Initializing Agentic Scout cluster...",
    "Handshake complete. Workspace ready."
  ];
      ...
      <nav className="fixed top-0 w-full z-[100] px-6 py-4 flex justify-between items-center bg-slate-950/40 backdrop-blur-xl border-b border-white/5">
        <div className="flex items-center gap-3 group cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[7px] text-medical-500 font-mono tracking-[0.4em] uppercase font-black">Autonomous Research</span>
          </div>
        </div>
        <div className="flex items-center gap-6">
           <div className="hidden lg:flex gap-6 text-[8px] font-black uppercase tracking-[0.4em] text-slate-500">
              ...
              <a href="#pipeline" className="hover:text-medical-400 transition-colors">Stack</a>
              <a href="#integrity" className="hover:text-medical-400 transition-colors">Security</a>
           </div>
           <button 
      ...
      <main ref={containerRef} className="relative z-10">
          ...
          <div className="max-w-4xl mx-auto relative z-10">
            <div className="flex justify-center mb-6">
               <div className="px-3 py-1 rounded-full bg-slate-900/60 border border-white/5 backdrop-blur-md flex items-center gap-2">
                  ...
                  <span className="text-[8px] font-black text-slate-500 uppercase tracking-[0.4em]">Kernel_v3.5_Live</span>
               </div>
            </div>
            <h1 className="text-4xl md:text-7xl font-black tracking-tight mb-6 text-white leading-tight">
              Autonomous <br />
              <span className="italic font-serif text-transparent bg-clip-text bg-gradient-to-r from-medical-400 to-indigo-500">Clinical Research.</span>
            </h1>
            <p className="text-slate-400 text-base md:text-lg max-w-xl mx-auto font-medium leading-relaxed mb-10 tracking-tight">
              A high-fidelity research agent mimicking the rigorous cognitive process of clinical research boards. From synthesis to trial design.
            </p>
        ...
        <section id="benefits" className="py-24 px-6">
          <div className="max-w-5xl mx-auto">
             <SectionHeader 
               tag="The Clinical Advantage" 
               title="Built for Rigor." 
               desc="Engineered to meet the absolute requirements of clinical evidence synthesis."
             />
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  { icon: Target, title: 'Zero Hallucination', desc: 'Every claim is validated against NIH and PubMed databases. Non-grounded logic is purged.' },
                  { icon: Clock, title: 'Extreme Velocity', desc: 'Compress months of systematic review into seconds. Focus on discovery, not paperwork.' },
                  { icon: ShieldCheck, title: 'Certified Output', desc: 'Outputs are IMRAD-compliant and MeSH-standardized. Ready for institutional review.' }
                ].map((benefit, i) => (
        ...
        <section id="pipeline" className="py-24 px-6 bg-slate-950/40">
          <div className="max-w-5xl mx-auto text-center">
             <SectionHeader 
               tag="The Discovery Stack" 
               title="Autonomous Clusters." 
               desc="A synchronized multi-agent environment mimicking an MD/PhD board."
             />
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                { icon: Search, name: 'Agent Scout', metrics: '120 Papers/min' },
                { icon: Beaker, name: 'Agent Pathologist', metrics: '99% Veracity' },
                { icon: Workflow, name: 'Agent Protocol', metrics: 'ISO Optimized' },
                { icon: ShieldAlert, name: 'Agent Guard', metrics: 'Zero Noise' }
              ].map((agent, i) => (
        ...
        <section className="py-32 px-6 text-center">
           <div className="max-w-3xl mx-auto">
              <h2 className="text-4xl md:text-6xl font-black tracking-tight mb-8 text-white">
                Start Your Next <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-medical-400 to-indigo-500 italic font-serif">Research Session.</span>
              </h2>
              <button 
                onClick={() => setShowAuthModal(true)}
              ...
              >
                Initialize Kernel <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </button>
           </div>
        </section>
      </main>
          ...
          <div className="bg-slate-900 border border-white/10 rounded-[2.5rem] p-10 max-w-md w-full relative z-10 shadow-2xl overflow-hidden">
            {isScanning && (
              <div className="absolute inset-0 z-50 bg-[#020617] p-10 flex flex-col items-center justify-center animate-in fade-in">
                 ...
                 <h4 className="text-lg font-black text-white uppercase tracking-[0.4em] mb-4">Booting_Kernel</h4>
            ...
            <div className="text-center mb-8">
               ...
               <h3 className="text-2xl font-black mb-2 uppercase text-white tracking-tight">Handshake</h3>
               <p className="text-slate-500 text-base font-medium">Certify compliance to clinical governance standards.</p>
            </div>
            <div className="space-y-6">
               <div className="p-6 rounded-2xl bg-slate-950 border border-white/5">
                  <p className="text-[12px] text-slate-500 leading-relaxed italic mb-6">
                    I acknowledge that MediMind is an autonomous orchestration layer and that final human verification is mandatory.
                  </p>
                  <label className="flex items-center gap-4 cursor-pointer group/check">
                    ...
                    <span className="text-base font-bold text-slate-300">I accept governance standards.</span>
                  </label>
               </div>
               <button 
                onClick={handleStart} 
               ...
               >
                 ENTER <Zap className="w-5 h-5 fill-current" />
               </button>
            </div>
          </div>
        </div>
